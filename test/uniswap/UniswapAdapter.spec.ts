...
describe("xDonate", function () {
  ...
  before(async () => {
    ...
    const adapter = await UniswapAdapter.deploy();
    // const { xDonate } = await deployments.fixture(["UniswapAdapter"]);
    // donation = new Contract(xDonate.address, xDonate.abi, ethers.provider);

    // setup tokens
    tokenA = new Contract(DONATION_ASSET, ERC20_ABI, ethers.provider);
  ...
  describe("receiving funds", () => {
    ...
    it("should be able to receive donating asset", async () => {
      // Get initial balances
      const initialWallet = await tokenA.balanceOf(whale.address);
      const initialDonation = await tokenA.balanceOf(donation.address);

      // Send funds to the contract
      const amount = utils.parseUnits("1", DONATION_ASSET_DECIMALS);
      await fund(tokenA.address, amount, whale, donation.address);

      // Check balances
      expect((await tokenA.balanceOf(whale.address)).toString()).to.be.eq(initialWallet.sub(amount).toString());
      expect((await tokenA.balanceOf(donation.address)).toString()).to.be.eq(initialDonation.add(amount).toString());
    });
  ...
  describe("sweeping", () => {
    ...
    it("should work for donation asset", async () => {
      ...
      // send sweep tx
      const sweeping = await tokenA.balanceOf(donation.address);
      ...
      expect((await tokenA.balanceOf(donation.address)).toString()).to.be.eq("0")
      expect((await tokenA.balanceOf(CONNEXT)).toString()).to.be.eq(initConnext.add(sweeping));
    });
